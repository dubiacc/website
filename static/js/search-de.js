...
function searchAndDisplayArticles(e) {
    ...
    var no_results = "<p id='no-results' style='padding-left:10px;'>Keine Ergebnisse gefunden.</p>"
    if (searchterm.length == 0) {
        target.innerHTML = "";
        return false;
    } else if (searchterm.length < 3) {
...
function checkArticlesAreInitialized(force) {
    ...
    for (var id in window.articles.articles) {
        ...
        console.log("downloading index.md file for de/" + id);
        doAjax({
            location: `${location.origin}/articles/de/${id}/index.md`,
...
function initSearchIndex(force) {
    ...
    var version = "184c14c3cf1ba810a2bd681ceb117f3290f647e2";
    ...
    console.log("initSearchIndex: downloading /de/index.json");

    // TODO: diff and delete outdated articles
    doAjax({
        location: `${location.origin}/de/index.json`,
        onSuccess: function(event) {
