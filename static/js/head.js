...
AuxLinks = {
    auxLinksLinkTypes: {
        ...
        "/metadata/annotation/similar/":            "similar",
        "/metadata/annotation/bibliography/":  "bibliography"
    },
...
Annotations = { ...Annotations,
	...
	referenceDataFromParsedAPIResponse: (response, link) => {
		...
		let similarElement = response.querySelector(".similar");
		let similarHTML = similarElement
						   ? `<span
							   class="data-field aux-links similar"
							   >${similarElement.innerHTML}</span>`
						   : null;

		//	The link-bibliography link (if exists).
		let linkbibElement = response.querySelector(".bibliography");
		let linkbibHTML = linkbibElement
						  ? `<span
							  class="data-field aux-links bibliography"
							  >${linkbibElement.innerHTML}</span>`
						  : null;

		//	All the aux-links (tags, backlinks, similar, link link-bib).
		let auxLinksHTML = ([ backlinksHTML, similarHTML, linkbibHTML ].filter(x => x).join(", ") || null);
...
/*  Transclusion is dynamic insertion, into a document, of part or all of
	...
	class="include-content-core"
		...
		data-include-selector-not="#footnotes, #backlinks, 
...
function includeContent(includeLink, content) {
	...
	if (   transcludingIntoFullPage
		...
		&& wrapper.closest("#backlinks") != null)
		...
		if (link.classList.containsAnyOf([ "include-spinner", "include-spinner-not" ]) == false) {
			...
			if (isWithinCollapsedBlock(link) == false) {
				...
				if (   containingAuxLinksBlock
					&& containingAuxLinksBlock.classList.contains("bibliography-list")) {
					link.classList.add("include-spinner");
				}
			}
		}
...
Extracts = { ...Extracts,
    ...
    preparePopup_ANNOTATION: (popup) => {
        ...
        let targetAnalogueInLinkBibliography = document.querySelector(`a[id^='bibliography'][href='${escapedLinkURL}']`);
...
addContentInjectHandler(GW.contentInjectHandlers.applyLinkBibliographyCompactStylingClass = (eventInfo) => {
    ...
    eventInfo.container.querySelectorAll(".bibliography-list").forEach(linkBibList => {
        if (linkBibList.closest("li, .bibliography-append, .popframe-body.bibliography"))
            linkBibList.classList.add("bibliography-list-compact");
    });
}, "rewrite");
...
window.addEventListener("beforeprint", GW.beforePrintHandler = (event) => {
    ...
    function expand(container) {
        Transclude.allIncludeLinksInContainer(container).forEach(includeLink => {
            if (includeLink.closest("#bibliography, .bibliography-append"))
                return;
