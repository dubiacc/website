...
def extract_text_from_html(
    ...
    try:
        if USE_WEBDRIVER_MANAGER and webdriver_path is None:
            # ChromeDriverManager.install() can be called in each process, it caches the driver
            service = Service(ChromeDriverManager().install())
        elif webdriver_path:
            service = Service(webdriver_path)
        else:
            # This case should ideally be caught before multiprocessing starts,
        ...
        if author_css_selector:
            try:
                ...
                # Print to stderr for immediate feedback in case of partial failure
        ...
        if empty_body:
            extracted_text = ["<HTML FILE DOES NOT CONTAIN ANY TEXT CONTENT>"]
        ...
        return True, f"Successfully extracted text to {os.path.basename(output_file_path)}"

    except FileNotFoundError as e:
        return False, f"Error: ChromeDriver or Chrome binary not found. {e}"
    except WebDriverException as e:
        return False, f"Selenium WebDriver error: {e}"
    except TimeoutException:
        return False, f"Page load timed out"
    except Exception as e:
...
def _worker_extract_html(
    html_file_path,
    output_dir,
    main_css_selector,
    chrome_path,
    webdriver_path, # Passed explicitly now
    author_css_selector,
    date_css_selector,
    title_css_selector
):
    """
    Wrapper function to pass arguments from the main process to extract_text_from_html
    and return the result along with the html_file_path for identification.
    """
    ...
    return success, message, html_file_path
...
def main():
    ...
    parser.add_argument(
        "--processes",
        type=int,
        default=8, # Default to 8 processes as requested
        help="Number of parallel processes to use for extraction (default: 8)."
    )
    ...
    # Pre-check for ChromeDriver if webdriver_manager is not used and path not provided
    if not USE_WEBDRIVER_MANAGER and not args.webdriver_path:
        print("Error: ChromeDriver path must be specified via --webdriver-path if webdriver_manager is not installed.", file=sys.stderr)
        print("Please install webdriver_manager ('pip install webdriver-manager') or provide --webdriver-path.", file=sys.stderr)
        sys.exit(1)
    ...
    for html_file_path in html_files:
        ...
        # Apply the same sanitization as in extract_text_from_html to check against existing files
        sanitized_expected_output_filename = re.sub(r'[\\/:*?"<>|]', '_', expected_output_filename)

        if sanitized_expected_output_filename in processed_output_filenames:
            skipped_extractions += 1
            # Using tqdm.write to print without disturbing the progress bar
            # tqdm.write(f"Skipped {html_base_name}: Output file '{sanitized_expected_output_filename}' already exists.")
        else:
            html_files_to_process.append(html_file_path)

    print(f"Skipped {skipped_extractions} files that appear to be already processed.")
    print(f"Will process {len(html_files_to_process)} files using {args.processes} processes.")
    print(f"Extracting text to '{args.output_dir}'...")

    successful_extractions = 0
    failed_extractions = []

    # Prepare arguments for the worker function. Each tuple is a set of args for one call.
    tasks = [
        (
            html_file,
            args.output_dir,
    ...
    if tasks: # Only create executor if there are tasks to process
        with ProcessPoolExecutor(max_workers=args.processes) as executor:
            # executor.map takes a function and multiple iterables for its arguments.
            # *zip(*tasks) unpacks the list of tuples into separate argument iterables.
            # tqdm wraps the iterator returned by executor.map for progress tracking.
            for success, message, html_file_path in tqdm(
                executor.map(_worker_extract_html, *zip(*tasks)),
                total=len(tasks),
                ...
                if success:
                    ...
                    failed_extractions.append((html_file_path, message))
                    # Print errors immediately without disturbing the progress bar
                    tqdm.write(f"Failed {os.path.basename(html_file_path)}: {message}") 
    else:
        print("No files to process in this run.")
...
if __name__ == "__main__":
    ...
    # and good practice on other OSes. It ensures that child processes
    # do not re-import and re-execute the main script's code, preventing
    # recursion issues.
    main()